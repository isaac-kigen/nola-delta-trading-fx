...
import { fxUniverse } from '@/lib/system-mock-data';
...
export default function MarketDataPage() {
  ...
  const [selectedSymbol, setSelectedSymbol] = useState(fxUniverse[0]);
  const selected = marketSnapshot.find((row) => row.symbol === selectedSymbol) ?? marketSnapshot[0];

  const miniSeries = useMemo(() => buildMiniSeries(selected.price), [selected.price]);
    ...
    <div className="space-y-6">
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader>
          ...
          <CardDescription>
            Tracked symbols match sync-latest-candle default list and strategy spread/volatility inputs
          </CardDescription>
        </CardHeader>
      </Card>
